...
from model.item2vec_v2 import Item2Vec, Options
...
with tf.Graph().as_default(), tf.compat.v1.Session() as session:
    with tf.device("/cpu:0"):
        ...
        f_tf = tf.convert_to_tensor(value=f, dtype=np.float64)
        ...
        saver = tf.compat.v1.train.Saver([factors])
        ...
        projector.visualize_embeddings(tf.compat.v1.summary.FileWriter(args.save_project_path), config)
