...
class Item2Vec(object):
    ...
    def _init_graphs(self):
        self.batch = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size])
        self.labels = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size])
        ...
        self.summary_writer = tf.compat.v1.summary.FileWriter(self.save_path,
    ...
    def forward(self, batch, labels):
        ...
        embed = tf.Variable(tf.random.uniform([self.vocab_size, self.embed_dim],
        ...
        example_emb = tf.nn.embedding_lookup(params=embed, ids=batch)

        # Weights for labels: [batch_size, embed_dim]
        true_w = tf.nn.embedding_lookup(params=softmax_w, ids=labels)
        # Biases for labels: [batch_size, 1]
        true_b = tf.nn.embedding_lookup(params=softmax_b, ids=labels)

        # Weights for sampled ids: [batch_size, embed_dim]
        sampled_w = tf.nn.embedding_lookup(params=softmax_w, ids=sampled_ids)
        # Biases for sampled ids: [batch_size, 1]
        sampled_b = tf.nn.embedding_lookup(params=softmax_b, ids=sampled_ids)

        # True logits: [batch_size, 1]
        true_logits = tf.reduce_sum(input_tensor=tf.multiply(example_emb, true_w), axis=1) + true_b
    ...
    def nce_loss(self, true_logits, sampled_logits):
        ...
        nce_loss_tensor = (tf.reduce_sum(input_tensor=true_xent) +
                           tf.reduce_sum(input_tensor=sampled_xent)) / self.batch_size
